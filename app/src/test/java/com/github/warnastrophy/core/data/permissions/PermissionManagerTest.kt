...
class PermissionManagerTest {
  ...
  fun returns_Granted_when_all_permissions_are_granted() {
    ...
    Mockito.mockStatic(ContextCompat::class.java).use { mocked ->
      mocked
          .`when`<Int> {
      ...
      Assert.assertTrue(result is PermissionResult.Granted)
    }
  }
  ...
  fun getPermissionResult_returns_Denied_when_a_permission_is_not_granted() {
    Mockito.mockStatic(ContextCompat::class.java).use { mocked ->
      ...
      Assert.assertTrue(result is PermissionResult.Denied)
      val denied = result as PermissionResult.Denied
      Assert.assertTrue(denied.deniedPermissions.contains(Manifest.permission.ACCESS_FINE_LOCATION))
    }
  }
  ...
  fun returns_Denied_when_permission_temporarily_denied() {
    ...
    Mockito.mockStatic(ContextCompat::class.java).use { mocked ->
      ...
      val spyActivity = Mockito.spy(activity)

      // Mock shouldShowRequestPermissionRationale to return true (simulate show rationale)
      Mockito.`when`(spyActivity.shouldShowRequestPermissionRationale(ArgumentMatchers.anyString()))
          .thenReturn(true)
  ...
  fun returns_PermanentlyDenied_when_permission_permanently_denied() {
    ...
    Mockito.mockStatic(ContextCompat::class.java).use { mocked ->
      ...
      Assert.assertTrue(result is PermissionResult.PermanentlyDenied)
  ...
  fun markPermissionsAsAsked_and_isPermissionAskedBefore_work_correctly() {
    ...
    Assert.assertFalse(manager.isPermissionAskedBefore(type))
    manager.markPermissionsAsAsked(type)
    Assert.assertTrue(manager.isPermissionAskedBefore(type))
  }
}
