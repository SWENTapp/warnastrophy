...
class HealthCardRepositoryImplEmulatorTest {
    ...
    fun crudHealthCard_forCurrentUser() = runTest {
        ...
        val live = repo.observeMyHealthCard()
            .firstOrNullWithTimeout()
        assertNotNull(live)
...
suspend fun <T> kotlinx.coroutines.flow.Flow<T>.firstOrNullWithTimeout(
    ms: Long = 3_000
): T? = withTimeoutOrNull(ms) { first() }
