...
class HealthCardRepositoryImpl(
  ...
  private fun resolvedUid(): String =
      auth?.currentUser?.uid
          ?: fallbackUidProvider?.invoke()
          ?: error("No auth user and no fallback UID")

  private fun docRef(id: String = resolvedUid()) = db.collection(collectionName).document(id)

  override fun observeMyHealthCard(): Flow<HealthCard?> = callbackFlow {
    val reg =
        docRef().addSnapshotListener { snap, _ ->
          val card =
              if (snap != null && snap.exists()) snap.toObject(HealthCard::class.java) else null
          trySend(card)
        }
  ...
  override suspend fun getMyHealthCardOnce(fromCacheFirst: Boolean): HealthCard? {
    ...
    val snap =
        runCatching { docRef().get(source).await() }
